...
def info(msg: str) -> None: print(f"{BLUE}→  {msg}{NC}")
...
def cmd_validate(args: argparse.Namespace) -> None:
    ...
    for filepath in sorted(files):
        ...
        if errors: print(f"{RED}❌ {filepath}{NC}")
        else: print(f"{GREEN}✅ {filepath}{NC}")
...
def cmd_generate(args: argparse.Namespace) -> None:
    ...
    if not errors: ok("Validation passed!")
    else: warn(f"Validation found {len(errors)} issues.")
