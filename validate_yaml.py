...
# Valid enum values
VALID_TYPES: List[str] = ['concept', 'guide', 'reference', 'checklist', 'project', 'roadmap', 'template', 'audit']
...
def validate_file(filepath: str) -> Tuple[List[str], List[str]]:
    ...
    try:
        ...
        # Required fields
        required_fields: List[str] = ['title', 'type', 'domain', 'level', 'status', 'created', 'updated']
...
def main() -> None:
    ...
    md_files: List[str] = [f for f in all_files if not any(x in f for x in ['.github', 'README.md', 'CONTRIBUTING.md'])]
